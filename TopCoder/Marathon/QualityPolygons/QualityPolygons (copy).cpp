...
#define TIME_LIMIT 23.0
#define TIME_LIMIT2 24.0
...
//        fprintf (stderr, "%lf %lf %lf\n", minyDist, maxyDist, minyDist / maxyDist);
        new_order.pb(n);
        curr = n;
        used[nInd] = true;
    }while(curr != start );
...
//                    if ( min(miny, radius[minyIndex]) / max(miny, radius[minyIndex]) >= maxRadDiff/1.2)
        ...
        vector <string> choosePolygons(vector <int> pts, int sidesDiff, int radiiDiff){
            ...
            clustering_ratio = 8;
            fprintf(stderr, "[MAIN] clustering ratio: %d\n", clustering_ratio);
            NUM_CLUSTERS = max((int)(points.size() / clustering_ratio), 5);
            fprintf (stderr, "Number of clusters: %d\n", NUM_CLUSTERS);
            max_cluster_size = 7;
            radius_ratio =8.0;
            fprintf(stderr, "[CLUSTERING2] max cluster size: %d\n", max_cluster_size);
            fprintf(stderr, "[CLUSTERING2] radius ratio: %lf\n", radius_ratio);
            ...
            while (true){
                ...
                NUM_CLUSTERS = pointsLeft.size() / clustering_ratio;
